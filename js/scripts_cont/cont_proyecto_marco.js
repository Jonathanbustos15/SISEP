  ...
  $("#btn_nuevoProyectoM").jquery_controllerV2({
        ...
        if (ajustes.action == "crear") {
          ...
          rel_municipios.serializa_array(rel_municipios.crea_array(rel_municipios.arrElementos,id_last_proyectoM));
           ...
           if (upload.arregloDeArchivos.length > 0) {
                  ...
                  $('#fileuploadPM').fileupload('send', {files:upload.arregloDeArchivos})
    ...
    $("[name*='edita_proyectoM']").jquery_controllerV2({
      ...
      functionAfter:function(data){
        ...
        //----------------------------------------------------------------
        var query = "SELECT municipio.*, proyectoM_municipio.pkID as numReg "+ 

                    " FROM `municipio` "+

                    " INNER JOIN proyectoM_municipio ON proyectoM_municipio.fkID_municipio = municipio.pkID "+

                    " INNER JOIN proyecto_marco ON proyectoM_municipio.fkID_proyectoM = proyecto_marco.pkID "+

                    " WHERE proyectoM_municipio.fkID_proyectoM = "+id_proyectoM;
